...
local function has_at_least_one_compatible_route_in_service(self, plugin, service_pk)
  local routes, err_t, offset
  repeat
    routes, err_t, offset = self.db.routes:page_for_service(service_pk, nil, offset)
    if not routes then
      return nil, err_t
    end

    for i = 1, #routes do
      if has_at_least_one_common_protocol_with_route(plugin, routes[i]) then
        return true
      end
    end
  until not offset
